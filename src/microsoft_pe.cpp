...
void microsoft_pe_t::mz_placeholder_t::_read() {
    ...
    m_cparhdr = m__io->read_u2le();
    m_minalloc = m__io->read_u2le();
    m_maxalloc = m__io->read_u2le();
    m_ss = m__io->read_u2le();
    m_sp = m__io->read_u2le();
    m_csum = m__io->read_u2le();
    m_ip = m__io->read_u2le();
    m_cs = m__io->read_u2le();
    m_lfarlc = m__io->read_u2le();
    m_ovno = m__io->read_u2le();
    int l_res = 4;
    m_res = new std::vector<uint16_t>();
    m_res->reserve(l_res);
    for (int i = 0; i < l_res; i++) {
        m_res->push_back(m__io->read_u2le());
    }
    m_oemid = m__io->read_u2le();
    m_oeminfo = m__io->read_u2le();
    int l_res2 = 10;
    m_res2 = new std::vector<uint16_t>();
    m_res2->reserve(l_res2);
    for (int i = 0; i < l_res2; i++) {
        m_res2->push_back(m__io->read_u2le());
    }
